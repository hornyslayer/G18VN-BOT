...
import nextcord, os, locale, math
...
class Economy(commands.Cog):
    ...
    async def on_message(self, ctx):
        ...
        if last_coins_time is None or now - last_coins_time > timedelta(seconds=60):
            ...
            self.last_coins[user.id] = now  # cập nhật thời gian cuối cùng người dùng nhận tiền
            print(f"{user} nhận được {reward} vàng")
    def format_money(self, amount):
        locale.setlocale(locale.LC_ALL, '') # cập nhật locale theo ngôn ngữ máy tính
    ...
    async def create_balance(self, user):
        async with commands.db.cursor() as cursor:
            await cursor.execute("INSERT INTO bank VALUES(?, ?, ?, ?)", (0, 100, 100000, user.id))
        await commands.db.commit()
        return
    ...
    async def balance(self,ctx: commands.Context, member: nextcord.Member = None):
        ...
        em.add_field(name=":money_with_wings: Tiền mặt:", value = self.format_money(wallet))
        em.add_field(name=":bank: Ngân hàng:", value=f"{self.format_money(bank)}/{self.format_money(maxbank)}")
        em.add_field(name=":moneybag: Tổng tài sản:", value=f"{self.format_money(wallet+bank)}")
        if member:
    ...
    async def bal_cmd(self,ctx, member: nextcord.Member = None):await self.balance(ctx, member)
    ...
    async def withdraw(self,ctx: commands.Context, amount):
        ...
        if bank_res == 0 or wallet_res == 0:
            return await ctx.send("Oong beo")
        elif bank_res == 1:
            return await ctx.send("Oong beo 2")
        em = nextcord.Embed(description=f":white_check_mark: Rút thành công **{self.format_money(amount)}** {coin} về ví", color=done)
        em.set_author(name=ctx.author, icon_url=ctx.author.avatar.url)
    ...
    async def deposit(self,ctx: commands.Context, amount):
        ...
        if bank_res == 0 or wallet_res == 0:
            ...
            return await ctx.send("Oong beo 2")
        em = nextcord.Embed(description=f":white_check_mark: Gửi thành công **{self.format_money(amount)}** {coin} vào ngân hàng.", color=done)
        em.set_author(name=ctx.author, icon_url=ctx.author.avatar.url)
        await ctx.reply(embed=em)
    ...
    async def give(self,ctx: commands.Context, member: nextcord.Member, amount):
        ...
        em = nextcord.Embed(description=f":white_check_mark: {member.name} đã nhận **{self.format_money(amount)}** {coin}", color=done)
        em.set_author(name=ctx.author, icon_url=ctx.author.avatar.url)
        await ctx.reply(embed=em)
    ...
    async def rob(self,ctx: commands.Context, member: nextcord.Member):
        if random.random() < 0.7:
            ...
            em = nextcord.Embed(description=f":x: {ctx.author} không thành công khi cố gắng trộm {member.name} và bị phạt {self.format_money(fail_amount)} {coin}", color=fail)
            em.set_author(name=ctx.author, icon_url=ctx.author.avatar.url)
        ...
        em = nextcord.Embed(description=f":white_check_mark: {member.name} đã bị {ctx.author} trộm mất **{self.format_money(rob_amount)}** {coin}", color=done)
        em.set_author(name=ctx.author, icon_url=ctx.author.avatar.url)
        await ctx.reply(embed=em)
    ...
    async def work(self, ctx: commands.Context):
        ...
        em = nextcord.Embed(description=f":white_check_mark: {description} và kiếm được **{self.format_money(amount)}** {coin}", color=0x2ecc71)
        em.set_author(name=ctx.author, icon_url=ctx.author.avatar.url)
        await ctx.reply(embed=em)
    ...
    async def daily(self,ctx: commands.Context):
        ...
        await self.update_wallet(ctx.author, amount)
        em = nextcord.Embed(description=f":white_check_mark: Điểm danh ngày thành công bạn nhận được **{self.format_money(amount)}** {coin}", color=done)
        em.set_author(name=ctx.author, icon_url=ctx.author.avatar.url)
        await ctx.reply(embed=em)
    ...
    async def weekly(self,ctx: commands.Context):
        ...
        await self.update_wallet(ctx.author, amount)
        em = nextcord.Embed(description=f":white_check_mark: Điểm danh tuần thành công bạn nhận được **{self.format_money(amount)}** {coin}", color=done)
        em.set_author(name=ctx.author, icon_url=ctx.author.avatar.url)
        await ctx.reply(embed=em)
    #Coin Flip
    ...
    async def coinflip(self,ctx, choice, amount):
        ...
        em = nextcord.Embed(title=f"{ctx.author}", description=f"Tung đồng xu...", color=waiting)
        msg = await ctx.reply(embed=em)
        await asyncio.sleep(2)
        if side.lower() == choice.lower():
            await self.update_wallet(ctx.author, amount*2)
            em = nextcord.Embed(title=f"{side}", description=f"Bạn đã đoán đúng! Bạn nhận được **{self.format_money(amount*2)}** {coin}", color=done)
            em.set_author(name=ctx.author, icon_url=ctx.author.avatar.url)
            await msg.edit(embed=em)
        else:
            await self.update_wallet(ctx.author, -amount)
            em = nextcord.Embed(title=f"{side}", description=f"Rất tiếc, bạn đã đoán sai. Bạn mất **{self.format_money(amount)}** {coin}", color=fail)
            em.set_author(name=ctx.author, icon_url=ctx.author.avatar.url)
            await msg.edit(embed=em)
    @commands.command(name='cf')
    async def coinflip_cmd(self,ctx, choice, amount):await self.coinflip(ctx, choice, amount)
